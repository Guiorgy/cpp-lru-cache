...
namespace guiorgy {
	...
	public:
		...
		void put(const key_t& key, ValueArgs&&... value_args) {
			put<likelihood(key_likely_exists), likelihood(cache_likely_full)>(key, std::forward<ValueArgs>(value_args)...);
		}
		template<const bool key_likely_exists, typename... ValueArgs>
		void put(const key_t& key, ValueArgs&&... value_args) {
			put<likelihood(key_likely_exists)>(key, std::forward<ValueArgs>(value_args)...);
		}
