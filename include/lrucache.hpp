...
public:
	...
	lru_cache(const lru_cache& other) {
		...
		for (map_iterator_t it = _cache_items_list.begin(); it != _cache_items_list.end(); ++it) {
...
public:
	...
	const value_t& emplace(const key_t& key, ValueArgs&&... value_args) {
		const value_t& value = _cache_items_list.emplace_front(key, std::forward<ValueArgs>(value_args)...);
		put(key);
		return value;
	}
