...
namespace guiorgy::detail {
	...
	enum struct TypeQualifier : TypeQualifierBaseType {
		...
		Pointer                 = 0b0000'0010u,                 // T*
		Reference               = 0b0000'0100u,                 // T&
		RValueReference         = 0b0000'1000u,                 // T&&
...
namespace guiorgy::detail {
	...
	public:
		...
		// Piecewise emplace constructor.
		template<typename... KeyArgs, typename... ValueArgs>
		constexpr key_value([[maybe_unused]] std::piecewise_construct_t, std::tuple<KeyArgs...> key_args, std::tuple<ValueArgs...> value_args)
			noexcept(
				noexcept(emplace_new<key_type, KeyArgs...>(key, std::move(key_args)))
				&& noexcept(emplace_new<value_type, ValueArgs...>(value, std::move(value_args)))
			) {
			emplace_new<key_type, KeyArgs...>(key, std::move(key_args));
	...
	class lru_cache_opts final {
		...
		public:
			...
			constexpr hashmap_const_iterator_wrapper& operator=(const hashmap_iterator_type it)
				noexcept(std::is_nothrow_copy_assignable_v<hashmap_const_iterator_type, hashmap_iterator_type>) {
				it_ = it;
				return *this;
			}
			...
			constexpr hashmap_const_iterator_wrapper& operator=(const hashmap_const_iterator_type it)
				noexcept(std::is_nothrow_copy_assignable_v<hashmap_const_iterator_type, hashmap_const_iterator_type>) {
				it_ = it;
				return *this;
			}
