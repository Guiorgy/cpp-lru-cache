...
namespace guiorgy::detail {
	...
	struct _make_index_sequence_range<N, M, typename std::enable_if<(N < M)>::type> final {
	private:
	...
	template <std::size_t N, std::size_t M>
	struct _make_index_sequence_range<N, M, typename std::enable_if<(M <= N)>::type> final {
...
namespace std {
	...
	struct numeric_limits<guiorgy::detail::unpromoting<int_t, promoted_t>> {
		...
		static constexpr int radix = std::numeric_limits<int_t>::radixradix;
...
namespace guiorgy::detail {
	...
	class lru_cache_opts final {
		...
		private:
			const hashmap_const_iterator_type it_;
